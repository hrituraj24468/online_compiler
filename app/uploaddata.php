...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    $insertionquery="SELECT * FROM `code` WHERE username='$username' AND filename='untitled';";
  $result=mysqli_query($conn,$insertionquery); 
  $num=mysqli_num_rows($result);
  if($num==1)
  {
    
        $sql = "UPDATE `code` SET `code` = '$code' WHERE `username` = '$username' AND `filename` = 'untitled'";
    ...
    if(!isset($_SESSION['filename'])){
        $sql = "INSERT INTO `code` ( `username`, `code`, `filename`) VALUES ('$username', '$code', 'untitled')";
        $result = mysqli_query($conn, $sql);
        if ($result){
            $output=$code;
